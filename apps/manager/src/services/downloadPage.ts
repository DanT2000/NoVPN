...
<div class="wrap">
  ...
  <details class="tshoot">
    ...
    <p>
      Расширение ещё проходит проверку в магазинах. Пока его ставит само приложение NoVPN:
      оно держит распакованную копию в своей папке и обновляет её — при запуске и раз в час.
      Вам нужно один раз указать эту папку браузеру.
    </p>
    <p><b>Chrome, Edge, Яндекс Браузер:</b></p>
    <ol>
      <li>В приложении NoVPN: <b>Настройки &rarr; Расширение для браузера &rarr; Скачать расширение</b>.
        Появится путь к папке и кнопки «Открыть папку» и «Скопировать путь».</li>
      ...
      <li>Нажать <b>Загрузить распакованное расширение</b> и указать ту самую папку
        (обычно <code>%APPDATA%\\NoVPN\\extension\\chrome</code>).</li>
      <li>Значок появится на панели. Приложение NoVPN должно быть запущено. Когда папка
        обновится, нажмите «Обновить» на странице расширений или перезапустите браузер.</li>
    </ol>
    <p><b>Firefox:</b></p>
    <ol>
      <li>В приложении NoVPN нажать <b>Папка для Firefox</b> (там же, в настройках).</li>
      <li>Открыть <code>about:debugging#/runtime/this-firefox</code>.</li>
      <li>Нажать <b>Загрузить временное дополнение</b> и выбрать <code>manifest.json</code> из этой папки.</li>
      <li>В <code>about:addons</code> &rarr; NoVPN &rarr; «Разрешения» включить доступ ко всем сайтам —
        иначе попутные домены предлагаться не будут.</li>
      <li>Firefox снимает такие дополнения при перезапуске — до публикации в каталоге это
        нормально.</li>
    </ol>
    <p>
      Без приложения архивы можно взять напрямую:
      <a href="/extension/novpn-extension-chrome.zip">для Chrome, Edge и Яндекса</a>,
      <a href="/extension/novpn-extension-firefox.zip">для Firefox</a> — распаковать в постоянную
      папку и указать её так же.
    </p>
  </details>
</div>
</body></html>`;
