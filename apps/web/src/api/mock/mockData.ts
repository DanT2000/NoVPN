...
export const USERS: User[] = [
  { id: 'u1', name: 'Иван', comment: 'Брат', category: 'Семья', tags: ['родные'], code: '482915', accessToken: 'tok-482915', codeLoginUntil: null, deviceLimit: 5, expiresAt: iso(now + 33 * D), trafficLimitGb: 100, trafficUsedGb: 18.4, resetPolicy: 'monthly', allowedServers: ['s1', 's2'], allowedProtocols: ['xray', 'amneziawg'], allowedProxies: [], fullServers: [], isActive:true, telegram: '@ivan_k', createdAt: iso(now - 60 * D), lastActivityAt: iso(now - 5 * 60000) },
  { id: 'u2', name: 'Мария', comment: '', category: 'Друзья', tags: ['промо'], code: '730114', accessToken: 'tok-730114', codeLoginUntil: null, deviceLimit: 3, expiresAt: iso(now + 3 * D), trafficLimitGb: 10, trafficUsedGb: 9.1, resetPolicy: 'never', allowedServers: ['s1'], allowedProtocols: ['xray'], allowedProxies: [], fullServers: [], isActive:true, telegram: null, createdAt: iso(now - 27 * D), lastActivityAt: iso(now - 2 * 3600000) },
  { id: 'u3', name: 'Офис-01', comment: 'Общий доступ для офиса', category: 'Работа', tags: ['офис', 'vip'], code: '205871', accessToken: 'tok-205871', codeLoginUntil: null, deviceLimit: null, expiresAt: null, trafficLimitGb: null, trafficUsedGb: 240.8, resetPolicy: 'never', allowedServers: ['s1', 's2', 's3'], allowedProtocols: ['xray', 'amneziawg'], allowedProxies: [], fullServers: [], isActive:true, telegram: '@office_admin', createdAt: iso(now - 120 * D), lastActivityAt: iso(now - 20 * 60000) },
  { id: 'u4', name: 'Пётр', comment: '', category: 'Друзья', tags: [], code: '664209', accessToken: 'tok-664209', codeLoginUntil: null, deviceLimit: 2, expiresAt: iso(now - 6 * D), trafficLimitGb: 50, trafficUsedGb: 31.2, resetPolicy: 'never', allowedServers: ['s1'], allowedProtocols: ['xray', 'amneziawg'], allowedProxies: [], fullServers: [], isActive:true, telegram: null, createdAt: iso(now - 90 * D), lastActivityAt: iso(now - 8 * D) },
  { id: 'u5', name: 'Тест', comment: 'Отключён вручную', category: 'Общие', tags: ['тест'], code: '918273', accessToken: 'tok-918273', codeLoginUntil: null, deviceLimit: 1, expiresAt: iso(now + 300 * D), trafficLimitGb: 5, trafficUsedGb: 0.2, resetPolicy: 'never', allowedServers: ['s1'], allowedProtocols: ['xray'], allowedProxies: [], fullServers: [], isActive:false, telegram: null, createdAt: iso(now - 14 * D), lastActivityAt: null },
  { id: 'u6', name: 'Анна', comment: '', category: 'Семья', tags: [], code: '555001', accessToken: 'tok-555001', codeLoginUntil: null, deviceLimit: 1, expiresAt: iso(now + 12 * D), trafficLimitGb: 20, trafficUsedGb: 20, resetPolicy: 'monthly', allowedServers: ['s1'], allowedProtocols: ['xray', 'amneziawg'], allowedProxies: [], fullServers: [], isActive:true, telegram: null, createdAt: iso(now - 40 * D), lastActivityAt: iso(now - 3 * D) },
];
