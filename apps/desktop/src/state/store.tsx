...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    // движка отключено флагами WebView2 (--disable-background-timer-throttling и др.
    // в tauri.conf.json), поэтому опрос идёт и в фоне; частим до 0.7с, чтобы выбор
    // на сайте применялся почти сразу. Плюс перечитываем немедленно при возврате окна.
    const wake = () => {
