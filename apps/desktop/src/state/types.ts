...
/** Для сайтов маршрут ровно один из двух. */
export type Route = 'vpn' | 'direct';

/** Для приложения есть третий вариант — «Авто»: приложение не форсируется ни в
    туннель, ни мимо него, а следует общим правилам (сайты и списки). В движок
    уходит только «vpn» и «direct» (списки процессов); «auto» — это отсутствие
    правила, поэтому оно ничего не ломает. */
export type AppRoute = Route | 'auto';
