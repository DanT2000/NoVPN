...
Поле `key` у Chrome/Edge/Яндекса ОСТАВЛЯЕМ. Пока расширение ставится вручную
(«загрузить распакованное»), именно `key` закрепляет за ним постоянный идентификатор
fcigljfl…: без него каждый компьютер и профиль получают свой случайный ID, браузер не
пускает расширение к нативному хосту (allowed_origins не совпадает), и человек видит
«приложение не отвечает». Это и была причина «не коннектится на других ПК».
Магазину `key` не мешает: Chrome Web Store выводит из него тот же ID, так что и после
публикации идентификатор не поменяется. Убираем `key` только у Firefox — он его не
понимает и живёт своим gecko.id.
"""
import json
import shutil
import zipfile
...
def chromium_manifest() -> dict:
    # `key` НЕ трогаем: он держит постоянный ID при ручной установке (см. шапку).
