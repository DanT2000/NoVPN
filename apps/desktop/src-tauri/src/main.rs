...
use novpn_desktop::{cmds, deeplink, enginehost, extension, host, selftest, update};
...
fn main() {
    // Медиа-клавишный хук движка (из-за которого при активном окне Scroll Lock не
    // доходил до скриншот-тулов вроде PostShot) отключаем флагами WebView2. Через
    // переменную окружения это не работает — Tauri передаёт движку свои
    // additionalBrowserArgs и перекрывает её, поэтому флаги живут в tauri.conf.json
    // (app.windows[].additionalBrowserArgs), одним списком с дефолтными фичами.
    ...
    // Привилегированный фоновый движок VPN (режим адаптера) — отдельный headless
    // режим без окна, его поднимает задача планировщика с правами. Обычный запуск
    // окна-интерфейса сюда не заходит и остаётся с обычными правами, чтобы не
    // глушить чужие горячие клавиши по UIPI.
    #[cfg(windows)]
    if enginehost::requested() {
        enginehost::run();
        return;
    }
