...
> **Осторожно с кнопкой «Повторить первоначальную настройку».** Она не просто открывает мастер заново — она **сбрасывает всё**: удаляет подписку и все ваши правила (приложения и сайты) и возвращает к первому запуску. Ссылку на подписку придётся вставлять заново.
...
- **DNS-сервер** — Cloudflare, Google или Quad9. DNS-запросы (запросы адресов сайтов) идут по DNS-over-HTTPS, чтобы провайдер не подменял ответы.
