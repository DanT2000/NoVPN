...
export interface PublicBootstrapData {
  ...
  /** Продвинутый режим X-Ray (обход белых списков + аварийный фоллбэк) включён
   *  глобально админом. Кабинет/бот тогда отдают подписку `/full`. По умолчанию true. */
...
export interface IssueDeviceResult {
  ...
  vpnKeyNote?: string;
  /** Xray: конфиг переиспользован (уже был у пользователя на этом сервере),
   *  новый не создавался — одна конфигурация работает на любом числе устройств. */
  reused?: boolean;
  /** Xray: адрес автообновляемой подписки пользователя (одна на все устройства). */
